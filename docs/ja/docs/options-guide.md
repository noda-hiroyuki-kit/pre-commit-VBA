...
### 2. 出力をシンプルな1フォルダ構成にしたい { #2-use-a-simple-single-folder-output-structure }

```console
uv run pre_commit_vba.py extract --disable-folder-annotation --exclude-extension
```

### 3. 既存リポジトリの命名に合わせたい { #3-match-naming-in-an-existing-repository }
